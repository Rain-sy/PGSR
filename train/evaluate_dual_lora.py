...
"""
======================================================================
Dual-Stream FLUX SR Evaluation + LoRA (companion to train_dual_lora.py)
======================================================================

Supports checkpoints produced by train_dual_lora.py:
  - ControlNet + pixel branch state dicts (same as train_dual_control)
  - Optional LoRA adapter on the FLUX Transformer (when the checkpoint
    contains `use_lora=True` and a `lora_state_dict` payload)

Non-LoRA checkpoints (from train_dual_control / train_dual_control_v1) are
loaded transparently — LoRA simply stays disabled.

Usage:
    python evaluate_dual_lora.py \
        --checkpoint checkpoints/dual_lora/xxx/best_model.pt \
        --hr_dir Data/DIV2K/DIV2K_valid_HR \
        --lr_dir Data/DIV2K/DIV2K_valid_LR_bicubic_X4 \
        --strength 1 --num_steps 20
"""
...
# PEFT is optional; only required when the checkpoint contains a LoRA adapter.
try:
...
# ============================================================================
# LoRA target module presets (kept in sync with train_dual_lora.py)
# ============================================================================
...
class DualStreamEvaluator(nn.Module):
    def __init__(self, model_name, device, checkpoint_path, pixel_weight=1.0):
        ...
        # LoRA bookkeeping (populated from checkpoint in load())
        self.use_lora = False
    ...
    def load(self):
        ...
        # Enable xformers on base models BEFORE LoRA wrap (same order as training)
        try:
        ...
        # ------------------------------------------------------------------
        # LoRA loading (opt-in based on checkpoint payload)
        # ------------------------------------------------------------------
        self.use_lora = bool(ckpt.get('use_lora', False)) and ('lora_state_dict' in ckpt)
        if self.use_lora:
            if not PEFT_AVAILABLE:
                raise ImportError(
                    "[LoRA] Checkpoint contains a LoRA adapter but `peft` is not "
                    "installed. Install with: pip install peft>=0.10"
                )
            ...
            ckpt_dropout = float(self.lora_config_ckpt.get('dropout', 0.0))
            ckpt_init = self.lora_config_ckpt.get('init_weights', 'gaussian')
            if ckpt_init in ('true', 'false'):
                ckpt_init = (ckpt_init == 'true')

            # Prefer the stored regex; fall back to the named preset.
            if not ckpt_regex and ckpt_preset in LORA_TARGET_PRESETS:
                ckpt_regex = LORA_TARGET_PRESETS[ckpt_preset]
                print(
                    f"[LoRA] Using target regex from preset '{ckpt_preset}' "
                    f"(regex not stored in checkpoint)."
                )
            elif not ckpt_regex:
                # Last-resort: OminiControl preset.
                ckpt_regex = LORA_TARGET_REGEX_OMINICONTROL
                print("[LoRA][WARN] No target_regex/target_preset in checkpoint; "
                      "falling back to ominicontrol preset.")

            print(
                f"[LoRA] Wrapping Transformer: rank={ckpt_rank}, alpha={ckpt_alpha}, "
                f"preset={ckpt_preset}, dropout={ckpt_dropout}"
            )
            lora_cfg = LoraConfig(
                r=ckpt_rank,
                lora_alpha=ckpt_alpha,
                target_modules=ckpt_regex,
                lora_dropout=0.0,  # eval-time: dropout always off
                bias="none",
            ...
            # Apply the saved LoRA state dict.
            lora_state = ckpt['lora_state_dict']
            # Move to correct device/dtype for loading.
            lora_state = {k: v.to(self.device) for k, v in lora_state.items()}
            missing_msg = set_peft_model_state_dict(self.transformer, lora_state)

            # Sanity: verify the adapter actually matched.
            wrapped = []
            ...
            print(
                f"[LoRA] Loaded {self.lora_state_tensors} adapter tensors into "
                f"{len(wrapped)} wrapped layers"
            )
            if missing_msg is not None and hasattr(missing_msg, 'missing_keys'):
                if missing_msg.missing_keys:
                    print(f"[LoRA][WARN] missing_keys: {len(missing_msg.missing_keys)}")
                if missing_msg.unexpected_keys:
                    print(f"[LoRA][WARN] unexpected_keys: {len(missing_msg.unexpected_keys)}")
            if len(wrapped) == 0 or self.lora_state_tensors == 0:
                raise RuntimeError(
                    "[LoRA] Adapter application failed: wrapped=0 or state_tensors=0. "
                    "Check that the checkpoint's target_regex/target_preset matches "
                    "the installed diffusers FLUX module names."
                )
        else:
            print("[LoRA] Checkpoint has no LoRA adapter; Transformer stays at base weights.")
...
def main():
    ...
    print("=" * 70)
    print("Dual-Stream FLUX SR Evaluation (LoRA-aware)")
    print("=" * 70)
