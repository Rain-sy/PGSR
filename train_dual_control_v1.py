...
def compute_flow_matching_loss(
    ...
    if do_lpips:
        ...
        try:
            ...
            # LPIPS decode can spike VRAM on large batches/resolution in Stage-2.
            # Instead of aborting the whole run, skip LPIPS for this step.
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            if not getattr(unwrapped, "_lpips_oom_warned", False):
                print(
                    "[LPIPS][OOM] Skipping LPIPS on this step due to CUDA OOM. "
                    "Consider smaller --batch_size / --resolution, or lower "
                    "--lpips_apply_prob."
                )
                setattr(unwrapped, "_lpips_oom_warned", True)
        finally:
