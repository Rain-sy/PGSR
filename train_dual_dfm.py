...
"""
======================================================================
Dual-Stream FLUX SR ControlNet Training -- DFM stage-grouped variant
======================================================================

A/B variant of ``train_dual_control.py``. Only real difference:
``PixelFeatureExtractor`` is refactored into four named submodules
(stage1 / stage2 / stage3_body / stage3_proj) with ``s3`` widened from
128 to 256 channels. Everything else (dataset, scheduler, LoRA, DFM
adapter wiring, loss, resume, eval) is intentionally kept identical so
results comparing this file to train_dual_control.py isolate the pixel
extractor refactor.

Incompatible with train_dual_control.py checkpoints (flat ``encoder.*``
keys will not match the new ``stage*`` keys). Designed for from-scratch
runs only.
...
class PixelFeatureExtractor(nn.Module):
    ...
    4. Checkpoints saved by train_dual_control.py (flat ``encoder.N.*`` keys)
       are NOT compatible with this class. Load will fail fast; no migration.
       This variant is designed for from-scratch runs only.
    """
...
def main():
    ...
    if args.resume:
        ...
        if 'pixel_extractor' in ckpt:
            ...
            # Fail-fast if someone tries to resume a flat-encoder checkpoint
            # saved by train_dual_control.py into this stage-grouped variant.
            # The keys look like ``encoder.0.weight``, which do not match
            # ``stage1.0.weight`` and would silently partial-load otherwise.
            if any(k.startswith('encoder.') for k in state.keys()):
                raise RuntimeError(
                    "[Resume][FATAL] Checkpoint pixel_extractor uses flat "
                    "``encoder.N.*`` keys (saved by train_dual_control.py). "
                    "train_dual_dfm.py uses the stage-grouped layout "
                    "(stage1/stage2/stage3_body/stage3_proj) and cannot load "
                    "these keys. Either resume with train_dual_control.py, or "
                    "drop --resume and train the DFM variant from scratch."
                )
