...
DEGRADATION_PRESETS = {
    'realesrgan': {
        'cfg': REALESRGAN_CFG,
        'usm': USM_CFG,
        'description': 'Current Real-ESRGAN-style high-order degradation.',
    },
    'realesrgan_mild': {
        'cfg': {
            **REALESRGAN_CFG,
            'blur_sigma': [0.2, 2.0],
            'blur_sigma2': [0.2, 1.0],
            'resize_range1': [0.25, 1.35],
            'resize_range2': [0.5, 1.1],
            'noise_sigma1': [1.0, 18.0],
            'noise_sigma2': [1.0, 12.0],
            'poisson_scale1': [0.05, 2.0],
            'poisson_scale2': [0.05, 1.5],
            'jpeg_range1': [45, 95],
            'jpeg_range2': [45, 95],
            'paired_prob': 0.20,
            'bicubic_prob': 0.15,
        ...
        'description': 'Milder Real-ESRGAN-style degradation for RealSR/DRealSR fidelity.',
    },
    'bsrgan': {
        'cfg': {
            **REALESRGAN_CFG,
            'pipeline': 'bsrgan',
            'blur_sigma': [0.2, 3.0],
            'resize_range1': [0.25, 1.5],
            'noise_sigma1': [1.0, 25.0],
            'poisson_scale1': [0.05, 2.5],
            'jpeg_range1': [35, 95],
            'paired_prob': 0.05,
            'bicubic_prob': 0.05,
        },
        ...
        'description': 'BSRGAN-inspired shuffled blur/downsample/noise/JPEG degradation.',
    },
    'real_paired_mix': {
        'cfg': {
            **REALESRGAN_CFG,
        ...
        'description': 'Synthetic degradation with a larger paired-real-data mix.',
    },
}


def get_degradation_preset(name):
    preset = DEGRADATION_PRESETS[name]
    return dict(preset['cfg']), dict(preset['usm']), preset['description']
...
class SRDataset(Dataset):
    ...
    def _degrade_realesrgan(self, hr_crop):
        cfg = self.realesrgan_cfg
        if cfg.get('pipeline') == 'bsrgan':
            return self._degrade_bsrgan(hr_crop)
    ...
    def _degrade_bsrgan(self, hr_crop):
        """BSRGAN-inspired shuffled degradation pipeline.

        The intent is not to reproduce every ISP detail, but to remove the
        fixed Real-ESRGAN operation order bias by shuffling blur, resize,
        noise, and JPEG before the final LR projection.
        """
        cfg = self.realesrgan_cfg
        s = self._blur_sigma_scale
        target_lr = self.resolution // self.scale
        ...
        def apply_blur(img, sigma_key='blur_sigma'):
            sigma_raw = cfg.get(sigma_key, [0.2, 3.0])
            sigma = [float(sigma_raw[0]) * s, float(sigma_raw[1]) * s]
            return self._random_blur(img, sigma)

        def apply_resize(img, prob_key='resize_prob1', range_key='resize_range1'):
            return self._random_resize(
                img,
                cfg.get(prob_key, [0.2, 0.7, 0.1]),
                cfg.get(range_key, [0.25, 1.5]),
                min_size=mid_min,
            )

        def apply_noise(img, prob_key='gaussian_noise_prob1', sigma_key='noise_sigma1',
                        poisson_key='poisson_scale1', gray_key='gray_noise_prob1'):
            return self._random_noise(
                img,
                float(cfg.get(prob_key, 0.5)),
                cfg.get(sigma_key, [1.0, 25.0]),
                cfg.get(poisson_key, [0.05, 2.5]),
                float(cfg.get(gray_key, 0.4)),
            )

        def apply_jpeg(img, key='jpeg_range1'):
            return self._jpeg_compress(img, cfg.get(key, [35, 95]))

        ops = [apply_blur, apply_resize, apply_noise, apply_jpeg]
        np.random.shuffle(ops)
        for op in ops:
            lq = op(lq)

        # A light second pass catches images that need compound artifacts while
        # keeping the preset milder than full high-order Real-ESRGAN.
        if np.random.rand() < float(cfg.get('bsrgan_second_pass_prob', 0.35)):
            ops2 = [
                lambda img: apply_blur(img, 'blur_sigma2'),
                lambda img: apply_resize(img, 'resize_prob2', 'resize_range2'),
                lambda img: apply_noise(img, 'gaussian_noise_prob2', 'noise_sigma2',
                                        'poisson_scale2', 'gray_noise_prob2'),
                lambda img: apply_jpeg(img, 'jpeg_range2'),
            ]
            np.random.shuffle(ops2)
            for op in ops2:
                lq = op(lq)

        lq = lq.resize((target_lr, target_lr), self._sample_resample())
        if np.random.rand() < float(cfg.get('final_sinc_prob', 0.5)):
            k_size = int(np.random.choice([7, 9, 11, 13, 15, 17, 19, 21]))
            cutoff = float(np.random.uniform(np.pi / 5.0, np.pi))
            lq = self._apply_kernel(lq, self._build_sinc_kernel(cutoff, k_size))
        lq = self._jpeg_compress(lq, cfg.get('jpeg_range2', cfg.get('jpeg_range1', [35, 95])))
        lq = Image.fromarray(self._clip_uint8(np.array(lq).astype(np.float32)), mode='RGB')
        return lq.resize((self.resolution, self.resolution), self._sample_resample())
...
class DualStreamFLUXSR(nn.Module):
    ...
    def __init__(self, model_name, device, pretrained_controlnet=None,
        ...
        # CLEAR config: when use_clear=True the FLUX dual-block attention
        # processors are swapped with local-window FlexAttn variants AFTER
        # LoRA injection so LoRA continues to receive gradient (CLEAR's
        # processor calls attn.to_q / attn.to_k via forward, which goes
        # through PEFT's LoRA wrapper).
        self.use_clear = bool(use_clear)
        self.clear_window_size = int(clear_window_size)
        ...
        self.train_clear_samplers = bool(train_clear_samplers)
        self.clear_lr = float(clear_lr)
        # Filled in by _init_clear; list of nn.Module processors so we can
        # collect their params for the optimizer.
        self._clear_processors = []

        # Learnable residual text embedding (zero-init delta on the empty
        # T5 prompt). Always present in this script so step-0 forward is
        # bit-identical to the v2/clear baseline (delta=0); from step 1 on
        # the network can learn a task-specific text prior.
    ...
    def _load_models(self, pretrained_controlnet):
        ...
        # Apply CLEAR local attention AFTER LoRA so the CLEAR processor's
        # forward calls go through the LoRA-wrapped attn.to_q / attn.to_k.
        if self.use_clear:
            self._init_clear(local_rank=local_rank)
    ...
    def _cache_text_embeddings(self):
        ...
        # Register the residual learnable text prior. Zero-init keeps the
        # forward bit-identical to the v2/CLEAR baseline at step 0.
    ...
    def _init_clear(self, local_rank=0):
        ...
        ``self.clear_ckpt`` and frozen; only LoRA on attn linears trains.

        Important: must be called AFTER ``_apply_lora_to_transformer`` so
        ``module.set_processor(...)`` operates on the PEFT-wrapped
        transformer (the processors call ``attn.to_q(x)`` via forward,
        which goes through LoRA).
        """
        try:
        ...
        dtype = torch.bfloat16
        # FLUX VAE downsample 8x then transformer packs 2x, so a 512x512
        # image becomes a 32x32 patch grid.
        patch_size = self.clear_resolution // 16
        device_str = str(self.device)

        if self.clear_down_factor > 1:
            init_local_downsample_mask_flex(
                height=patch_size, width=patch_size, text_length=512,
                window_size=self.clear_window_size,
        ...
        if not (self.clear_ckpt and os.path.exists(self.clear_ckpt)):
            if local_rank == 0:
                print(
                    f"[CLEAR][WARN] clear_ckpt not found ({self.clear_ckpt}); "
                    f"local-window processors will be set without preloaded "
                    f"weights (down/up samplers init from scratch)."
                )
            clear_weights = {}
        else:
            from safetensors.torch import load_file
        ...
        if local_rank == 0:
            ...
            print(
                ...
                f"patch_size={patch_size}x{patch_size})"
            )
    ...
    def get_lora_params(self):
        if not self.use_lora:
            return []
        clear_ids = set()
        for proc in getattr(self, '_clear_processors', []) or []:
            if hasattr(proc, 'parameters'):
                clear_ids.update(id(p) for p in proc.parameters())
        return [
            p for p in self.transformer.parameters()
            if p.requires_grad and id(p) not in clear_ids
        ]

    def get_clear_params(self):
        """Return CLEAR processor params (down/up samplers etc.) when they
        are trainable. Returns an empty list if CLEAR is disabled or
        ``train_clear_samplers=False``."""
        if not (self.use_clear and self.train_clear_samplers):
            return []
        out = []
        for proc in getattr(self, '_clear_processors', []) or []:
            if hasattr(proc, 'parameters'):
                out += [p for p in proc.parameters() if p.requires_grad]
        return out

    def get_text_embed_params(self):
        """Return the residual text-prior parameter list. Empty when frozen."""
    ...
    def forward(self, noisy, lr_lat, lr_pixel, timestep, guidance=3.5, controlnet_scale=1.0,
        ...
        # Text embeddings (cached empty CLIP pooled + cached empty T5 prompt
        # plus the zero-init learnable residual delta).
    ...
    def get_trainable_params(self):
        ...
        add_unique(self.get_pixel_branch_params())
        if self.train_controlnet:
            add_unique(self.controlnet.parameters())
        if self.use_lora:
            add_unique(self.get_lora_params())
        add_unique(self.get_clear_params())
        add_unique(self.get_text_embed_params())
        return params
...
def compute_flow_matching_loss(
    ...
    # L2 regularization on the residual learnable text delta (zero when
    # delta is frozen or text_embed_l2 <= 0).
...
def save_checkpoint(system, accelerator, epoch, loss, psnr, pixel_weight, strength,
    ...
    if isinstance(training_data, dict):
        payload['degradation_preset'] = training_data.get('degradation_preset')
        payload['degradation_cfg'] = training_data.get('degradation_cfg')
        payload['usm_cfg'] = training_data.get('usm_cfg')
    ...
    # CLEAR processor weights (down/up samplers) when CLEAR is on. We save
    # them as a list aligned with self._clear_processors order so resume
    # can restore them block-by-block.
    ...
    if getattr(unwrapped, 'learnable_text_delta', None) is not None:
        ...
        payload['text_embed_l2'] = float(unwrapped.text_embed_l2)
        payload['text_embed_lr'] = float(unwrapped.text_embed_lr)
        payload['text_embed_freeze'] = bool(unwrapped.text_embed_freeze)
        try:
...
def main():
    ...
    parser.add_argument('--degradation_preset', type=str, default='realesrgan',
                        choices=list(DEGRADATION_PRESETS.keys()),
                        help='Stage-2 degradation preset recorded in checkpoints.')
    parser.add_argument('--real_paired_hr_dirs', type=str, default='',
                        help='Comma-separated extra HR dirs for paired real-data mixing.')
    parser.add_argument('--real_paired_lr_dirs', type=str, default='',
                        help='Comma-separated extra LR dirs aligned with --real_paired_hr_dirs.')
    parser.add_argument('--real_paired_repeat', type=int, default=1,
                        help='Repeat extra paired real datasets this many times in the ConcatDataset.')
    ...
    parser.add_argument('--freeze_text_embed', dest='freeze_text_embed',
                        action='store_true', default=False,
                        help='Freeze the learnable T5 residual delta (ablation).')
    parser.add_argument('--text_embed_tokens', type=int, default=32,
                        help='Number of leading T5 tokens that get a learnable residual.')
    parser.add_argument('--text_embed_scale', type=float, default=0.1,
                        help='Scale on the residual delta added to the empty T5 prompt.')
    parser.add_argument('--text_embed_l2', type=float, default=1e-4,
                        help='L2 regularization on learnable_text_delta (0 disables).')
    parser.add_argument('--text_embed_lr', type=float, default=1e-6,
                        help='LR for learnable_text_delta param group.')
    parser.add_argument('--reset_text_embed', action='store_true', default=False,
                        help='On resume, reset learnable_text_delta to zero.')
    ...
    if args.real_paired_repeat < 1:
        parser.error("--real_paired_repeat must be >= 1")
    if args.degrade_mode == 'paired' and not args.lr_dir:
        parser.error("--lr_dir is required when --degrade_mode is 'paired'")
    real_paired_hr_dirs = [p.strip() for p in args.real_paired_hr_dirs.split(',') if p.strip()]
    real_paired_lr_dirs = [p.strip() for p in args.real_paired_lr_dirs.split(',') if p.strip()]
    if len(real_paired_hr_dirs) != len(real_paired_lr_dirs):
        parser.error("--real_paired_hr_dirs and --real_paired_lr_dirs must have the same number of entries")
    ...
    realesrgan_cfg, usm_cfg, degradation_preset_desc = get_degradation_preset(args.degradation_preset)
    ...
    exp_name = (
        ...
        f"_preset{args.degradation_preset}"
        f"_x{args.scale}"
    ...
    if args.use_clear:
        clear_train_tag = "trn" if args.train_clear_samplers else "frz"
        exp_name += (
            f"_clear{clear_train_tag}"
    ...
    if not args.freeze_text_embed:
        exp_name += (
            f"_text"
            f"_n{args.text_embed_tokens}"
            f"_s{_fmt_tag(args.text_embed_scale)}"
            f"_l{_fmt_tag(args.text_embed_l2)}"
        )
    ...
    train_data_info = {
        ...
        'degradation_cfg': dict(realesrgan_cfg),
        'usm_cfg': dict(usm_cfg),
        'real_paired_hr_dirs': list(real_paired_hr_dirs),
        'real_paired_lr_dirs': list(real_paired_lr_dirs),
        'real_paired_repeat': int(args.real_paired_repeat),
    }
    ...
    if is_main:
        ...
        print(f"Degradation Preset: {args.degradation_preset} ({degradation_preset_desc})")
        ...
        if args.use_clear:
            clr_state = "trainable" if args.train_clear_samplers else "frozen"
            print(f"CLEAR: ON (window={args.clear_window_size}, "
                  f"down_factor={args.clear_down_factor}, "
                  f"samplers={clr_state}, lr={args.clear_lr})")
        else:
            print("CLEAR: OFF (vanilla FLUX full attention)")
        if args.freeze_text_embed:
            print("Text Embed: FROZEN (ablation)")
        else:
            print(f"Text Embed: residual delta tokens={args.text_embed_tokens}, "
                  f"scale={args.text_embed_scale}, lr={args.text_embed_lr}")
        print(f"Geometric Aug: {args.geom_aug}")
        if real_paired_hr_dirs:
            print(f"Real Paired Mix: {len(real_paired_hr_dirs)} dirs x repeat {args.real_paired_repeat}")
    ...
    clear_params = system.get_clear_params()
    ...
    if len(clear_params) > 0:
        optimizer_grouped_parameters.append(
            {"params": clear_params, "lr": args.clear_lr}
        )
    if len(text_embed_params) > 0:
        optimizer_grouped_parameters.append(
            {"params": text_embed_params, "lr": args.text_embed_lr,
             "weight_decay": 0.0}
        )
    if is_main:
        clear_lr_str = f", clear={args.clear_lr:.2e}" if len(clear_params) > 0 else " (clear frozen)"
        ...
        print(
            ...
            f"lora={args.lora_lr:.2e}{clear_lr_str}{text_lr_str}"
        )
    ...
    if real_paired_hr_dirs:
        ...
        for _ in range(int(args.real_paired_repeat)):
            for extra_hr, extra_lr in zip(real_paired_hr_dirs, real_paired_lr_dirs):
                datasets.append(
                    SRDataset(
                        extra_hr, extra_lr, args.resolution,
                        num_crops=args.num_crops, is_val=False,
                        degrade_mode='paired', scale=args.scale,
                        realesrgan_cfg=realesrgan_cfg,
                        geom_aug=args.geom_aug,
                        usm_mode='off',
                        usm_weight=0.0,
        ...
        if is_main:
            print(
                f"[Data] ConcatDataset with synthetic/base + real paired mix: "
                f"{len(datasets)} shards, total samples={len(train_dataset)}"
            )
    ...
    if args.resume:
        ...
        # Restore learnable_text_delta from checkpoint when present and shapes
        # match. If absent (e.g. resume from a non-text v2 ckpt), keep zero
        # init -- forward stays bit-identical to baseline at step 0.
        ckpt_text_delta = ckpt.get('learnable_text_delta', None)
        if (
            unwrapped.learnable_text_delta is not None
            and ckpt_text_delta is not None
        ):
            try:
                src = ckpt_text_delta.to(
                    device=unwrapped.learnable_text_delta.device,
                    dtype=unwrapped.learnable_text_delta.dtype,
                )
                if src.shape == unwrapped.learnable_text_delta.shape:
                    with torch.no_grad():
                        unwrapped.learnable_text_delta.copy_(src)
                    if is_main:
                        print(f"[Resume] learnable_text_delta loaded "
                              f"(shape={tuple(src.shape)}).")
                elif is_main:
                    print(f"[Resume][WARN] learnable_text_delta shape mismatch: "
                          f"ckpt={tuple(src.shape)}, "
                          f"model={tuple(unwrapped.learnable_text_delta.shape)}; "
                          f"keeping fresh zero init.")
            except Exception as e:
                if is_main:
                    print(f"[Resume][WARN] failed to load learnable_text_delta: {e}")
        if args.reset_text_embed:
            unwrapped._reset_learnable_text_embed()
            if is_main:
                print("[Resume] --reset_text_embed: learnable_text_delta reset to zero.")

        # Restore CLEAR processor weights when present and CLEAR is on. If
        # absent (resume from non-CLEAR ckpt), keep the safetensors-loaded
        # weights from _init_clear() unchanged.
        ckpt_clear = ckpt.get('clear_processors', None)
        if unwrapped.use_clear and ckpt_clear is not None:
            try:
                ...
                if is_main:
                    print(f"[Resume][WARN] failed to load CLEAR processors: {e}")
    ...
    if is_main:
        with open(log_path, 'w') as f:
            ...
            f.write(f"Degradation Preset: {args.degradation_preset} ({degradation_preset_desc})\n")
            f.write(f"Degradation CFG: {realesrgan_cfg}\n")
            f.write(f"USM CFG: {usm_cfg}\n")
            if real_paired_hr_dirs:
                f.write(
                    f"Real Paired Mix: HR={real_paired_hr_dirs}, "
                    f"LR={real_paired_lr_dirs}, repeat={args.real_paired_repeat}\n"
                )
            ...
            if args.use_clear:
                clr_state = "trainable" if args.train_clear_samplers else "frozen"
                f.write(
                    f"CLEAR: ON (window={args.clear_window_size}, "
                    f"down_factor={args.clear_down_factor}, "
                    f"samplers={clr_state}, lr={args.clear_lr}, "
                    f"ckpt={args.clear_ckpt})\n"
                )
            else:
                f.write("CLEAR: OFF (vanilla FLUX full attention)\n")
            if args.freeze_text_embed:
                f.write("Text Embed: FROZEN (ablation)\n\n")
            else:
                f.write(
                    f"Text Embed: residual delta tokens={args.text_embed_tokens}, "
                    f"scale={args.text_embed_scale}, l2={args.text_embed_l2}, "
                    f"lr={args.text_embed_lr}\n\n"
                )
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        if is_main:
            ...
            if unwrapped.learnable_text_delta is not None:
                extra_tag += (
                    f", TextDeltaRMS={text_delta_rms:.6f}"
                    f", TextDeltaNorm={text_delta_norm:.4f}"
                )
