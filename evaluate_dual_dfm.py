...
class DualStreamEvaluator(nn.Module):
    def __init__(self, model_name, device, checkpoint_path, pixel_weight=1.0):
        ...
        # checkpoint sets use_learnable_text_embed=True). The parameter is
        # registered in `_cache_text_embeddings` so it has a sensible
        # warm-start fallback if the ckpt key is missing.
    ...
    def load(self):
        ...
        # peek set `self.use_learnable_text_embed=True`. Here we copy the
        # actual learned weights into it from the full ckpt.
    ...
    def _cache_text_embeddings(self):
        ...
        # register a Parameter with shape (1, 512, 4096) and warm-start from
        # the cached empty-string T5 output. The actual learned weights are
        # copied in below from `ckpt['learnable_text_embed']`. We keep the
        # warm-start fallback so eval still runs even if the key is missing.
    ...
    def forward(self, noisy, lr_lat, lr_pixel, timestep, guidance=3.5, controlnet_scale=1.0):
        ...
        # the checkpoint enabled it, else the cached empty-string T5 output.
        # CLIP `pooled` (temb path) and zero `text_ids` (pos embed path)
