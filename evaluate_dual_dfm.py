...
"""
======================================================================
Dual-Stream FLUX SR Evaluation -- DFM stage-grouped variant
======================================================================

Companion script for ``train_dual_dfm.py``. Uses the stage-grouped
PixelFeatureExtractor (stage1/stage2/stage3_body/stage3_proj with s3=256ch).

Not interchangeable with evaluate_dual_control.py -- checkpoints saved by
the flat-encoder trainer will fail to load here (and vice versa).

Usage:
    python evaluate_dual_dfm.py \
        --checkpoint checkpoints/dual_dfm/xxx/best_model.pt \
...
class PixelFeatureExtractor(nn.Module):
    """Inference-time pixel feature extractor -- stage-grouped variant.

    Mirror of the training version in ``train_dual_dfm.py``. Four named
    submodules (stage1 / stage2 / stage3_body / stage3_proj) with s3 widened
    to 256 channels. Incompatible with flat ``encoder.N.*`` checkpoints.
    """
    ...
    def load(self):
        ...
        if 'pixel_extractor' in ckpt:
            ...
            if any(k.startswith('encoder.') for k in state.keys()):
                raise RuntimeError(
                    "[Eval][FATAL] Checkpoint pixel_extractor uses flat "
                    "``encoder.N.*`` keys (saved by train_dual_control.py). "
                    "evaluate_dual_dfm.py expects the stage-grouped layout "
                    "(stage1/stage2/stage3_body/stage3_proj). Use "
                    "evaluate_dual_control.py for flat checkpoints, or train "
                    "a new checkpoint with train_dual_dfm.py."
                )
            self.pixel_extractor.load_state_dict(state, strict=True)
