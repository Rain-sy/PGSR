...
Recommended 2-stage training:

Stage 1) Paired bicubic — learn basic SR capability:
...
Stage 2) Real-ESRGAN degradation — adapt to real-world:
    accelerate launch --num_processes=8 --gradient_accumulation_steps=8 \
        train_dual_control.py \
        --hr_dir Data/Mix16K_HR \
...
class SRDataset(Dataset):
    ...
    # ------------------------------------------------------------------
    # Blur kernels (4 types following Real-ESRGAN)
    # ------------------------------------------------------------------

    @staticmethod
    def _build_gaussian_kernel_2d(kernel_size, sigma_x, sigma_y=None, rotation=0.0):
        """Build a 2D Gaussian kernel (isotropic when sigma_y is None or == sigma_x)."""
        if kernel_size % 2 == 0:
            kernel_size += 1
        if sigma_y is None:
            sigma_y = sigma_x
        ...
        rx = cos_r * xx + sin_r * yy
        ry = -sin_r * xx + cos_r * yy
        kernel = np.exp(-0.5 * (rx ** 2 / max(sigma_x ** 2, 1e-10) + ry ** 2 / max(sigma_y ** 2, 1e-10)))
        s = kernel.sum()
        if s > 0:
            kernel /= s
    ...
    def _build_generalized_gaussian_kernel(kernel_size, sigma, beta):
        """Generalized Gaussian kernel: exp(-0.5 * (|x|/sigma)^beta)."""
        if kernel_size % 2 == 0:
            kernel_size += 1
        half = kernel_size // 2
        ax = np.arange(-half, half + 1, dtype=np.float64)
        xx, yy = np.meshgrid(ax, ax)
        r = np.sqrt(xx ** 2 + yy ** 2)
        kernel = np.exp(-0.5 * (r / max(sigma, 1e-10)) ** max(beta, 1e-3))
        s = kernel.sum()
        if s > 0:
            kernel /= s
        return kernel.astype(np.float32)
    ...
    def _build_plateau_kernel(kernel_size, sigma, beta):
        """Plateau-shaped kernel: 1 / (1 + (|x|/sigma)^beta)."""
        if kernel_size % 2 == 0:
            kernel_size += 1
        half = kernel_size // 2
        ax = np.arange(-half, half + 1, dtype=np.float64)
        xx, yy = np.meshgrid(ax, ax)
        r = np.sqrt(xx ** 2 + yy ** 2)
        kernel = 1.0 / (1.0 + (r / max(sigma, 1e-10)) ** max(beta, 1e-3))
        s = kernel.sum()
        if s > 0:
            kernel /= s
        return kernel.astype(np.float32)

    def _random_blur(self, img, sigma_range, kernel_size=21):
        """Apply a random blur kernel (4 types following Real-ESRGAN)."""
        sigma = float(np.random.uniform(float(sigma_range[0]), float(sigma_range[1])))
        if sigma < 0.05:
            return img  # effectively no blur
        # Random kernel type: iso_gaussian / aniso_gaussian / generalized / plateau
        kernel_type = np.random.choice(
            ['iso_gaussian', 'aniso_gaussian', 'generalized_gaussian', 'plateau'],
            p=[0.45, 0.25, 0.12, 0.18],
        )
        if kernel_type == 'iso_gaussian':
            kernel = self._build_gaussian_kernel_2d(kernel_size, sigma)
        elif kernel_type == 'aniso_gaussian':
            sigma_y = float(np.random.uniform(float(sigma_range[0]), float(sigma_range[1])))
            rotation = float(np.random.uniform(0, np.pi))
            kernel = self._build_gaussian_kernel_2d(kernel_size, sigma, sigma_y, rotation)
        elif kernel_type == 'generalized_gaussian':
            beta = float(np.random.uniform(0.5, 4.0))
            kernel = self._build_generalized_gaussian_kernel(kernel_size, sigma, beta)
        else:  # plateau
            beta = float(np.random.uniform(1.0, 2.0))
            kernel = self._build_plateau_kernel(kernel_size, sigma, beta)
        return self._apply_kernel(img, kernel)

    @staticmethod
    def _build_sinc_kernel(cutoff, kernel_size):
        """Circular low-pass sinc kernel (2D Jinc / sombrero function)."""
        if kernel_size % 2 == 0:
            kernel_size += 1
        half = kernel_size // 2
        ax = np.arange(-half, half + 1, dtype=np.float64)
        xx, yy = np.meshgrid(ax, ax)
        r = np.sqrt(xx ** 2 + yy ** 2).astype(np.float64)
        omega = float(cutoff)
        # J1(omega*r) / (omega*r) — use Taylor expansion at r=0
        kernel = np.ones_like(r, dtype=np.float64)
        mask = r > 0
        omega_r = omega * r[mask]
        # Approximate J1 via sin/cos when scipy is unavailable:
        #   J1(x) ≈ sin(x - 3π/4) * sqrt(2/(πx))  for large x
        # For small-to-moderate x, use the series expansion.
        try:
            from scipy.special import j1
            kernel[mask] = j1(omega_r) / (omega_r)
        except ImportError:
            # Fallback: series J1(x) = x/2 - x^3/16 + x^5/384 - ...
            # Good for |x| < ~15, which covers typical kernel sizes
            val = np.zeros_like(omega_r)
            term = omega_r / 2.0
            val += term
            for n in range(1, 20):
                term *= -(omega_r ** 2) / (4.0 * n * (n + 1))
                val += term
            kernel[mask] = val / omega_r
        # Center value: lim_{r→0} J1(ωr)/(ωr) = 1/2
        kernel[~mask] = 0.5
    ...
    def _random_noise(self, img, gaussian_prob, sigma_range, poisson_scale_range, gray_prob):
        ...
        if np.random.rand() < float(gaussian_prob):
            ...
            if use_gray:
                # Compute noise from grayscale, then add correlated noise back to colour image
                gray = (0.299 * arr[..., 0] + 0.587 * arr[..., 1] + 0.114 * arr[..., 2]).astype(np.float32)
                noisy_gray = np.random.poisson(np.clip(gray, 0.0, 1.0) * 255.0 * scale).astype(np.float32) / (255.0 * scale)
                noise = noisy_gray - gray  # noise component only
                arr = arr + noise[..., None]  # broadcast (H,W) -> (H,W,3)
            else:
    ...
    # ------------------------------------------------------------------
    # USM (Unsharp Mask) sharpening for GT (following Real-ESRGAN)
    # ------------------------------------------------------------------
    @staticmethod
    def _usm_sharp(img, weight=0.5, threshold=10):
        """Apply USM sharpening: sharpened = img + weight * (img - blur(img)).

        Real-ESRGAN uses a Gaussian with sigma ≈ 0.3 (extremely narrow)
        so that only the very finest pixel-level details are enhanced.
        PIL GaussianBlur ``radius`` ≈ sigma, so we use radius = 1 here
        which is already slightly stronger than the original.  Setting
        radius any larger risks pushing GT away from the unsharpened
        validation images and tanking PSNR.
        """
        if weight <= 0:
            return img
        # radius=1 → sigma ≈ 1 pixel, mild edge sharpening only
        blur = img.filter(ImageFilter.GaussianBlur(radius=1))
        arr = np.array(img).astype(np.float32)
        arr_blur = np.array(blur).astype(np.float32)
        diff = arr - arr_blur
        # Only sharpen where the difference exceeds threshold (avoid amplifying noise)
        mask = np.abs(diff) > threshold
        arr[mask] = arr[mask] + weight * diff[mask]
        return Image.fromarray(np.clip(np.round(arr), 0, 255).astype(np.uint8), mode='RGB')

    # ------------------------------------------------------------------
    # Random geometric augmentation (flip + 90° rotation)
    # ------------------------------------------------------------------
    @staticmethod
    def _random_augment(*imgs):
        """Apply identical random H-flip, V-flip, 90° rotation to all images."""
    ...
    def __getitem__(self, idx):
        ...
        # USM sharpening on GT (training only, following Real-ESRGAN)
        if not self.is_val:
            hr_crop = self._usm_sharp(hr_crop)
